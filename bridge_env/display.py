...
class Display:
    ...
    def display_hands(self, hands):
        ...
        for p in PLAYERS:
            for i in range(4):
                x = 0
                for have, hand in zip(hands[p][i * 13:(i + 1) * 13],
                                      HAND_ARRAY):
                    if have == 1:
                        s[p][i][x] = hand
                        x += 1
    ...
    def display_bid_history(self):
        ...
        for i in range(h):
            if len(s['N']) > i * 4:
                print(' ' * wn,
                      ', '.join(s['N'][i * 4:min((i + 4) * h, len(s['N']))]))
            else:
                print()
        print()
        ...
        for i in range(h):
            if len(s['W']) > i * 4:
                print(' ' * ww,
                      ', '.join(s['W'][i * 4:min((i + 4) * h, len(s['W']))]))
            else:
                print()
        print()
        ...
        for i in range(h):
            if len(s['E']) > i * 4:
                print(' ' * we,
                      ', '.join(s['E'][i * 4:min((i + 4) * h, len(s['E']))]))
            else:
                print()
        print()
        ...
        for i in range(h):
            if len(s['S']) > i * 4:
                print(' ' * ws,
                      ', '.join(s['S'][i * 4:min((i + 4) * h, len(s['S']))]))
            else:
                print()
...
BID_ARRAY = ['{num}{suit}'.format(num=n, suit=s) for n in range(1, 8) for s in
             ['C', 'D', 'H', 'S', 'NT']] \
            + ['Pass', 'X', 'XX']
...
if __name__ == '__main__':
    ...
    for bid in [6, 7, 8, 12, 13, 14, 15, 16, 17, 35, 18, 36, 35, 19, 20, 35, 35,
                35]:
        b.take_bid(bid)
    print()
